...
class AdminController extends Controller
{
    ...
    public function CheckAccess(Request $request){
        Log::error('CheckAccess');
        $key = env('KEYACCESS');
        $req = $request->all();
        if(isset($req['email'])&&isset($req['activeKey'])&&isset($req['key'])){
            $mess = "Yêu cầu sử dụng mới từ:\n"
                . "<b>Email: </b>" . "$request->email\n"
                . "<b>activeKey: </b>" . "$request->activeKey\n"
                . "<b>key: </b>" . "$request->key\n";
            ...
            if($req['key']==$key){
                ...
                if($user){
                    if($user->status==1){
                        if($user->ip==$req['activeKey']){
                            if($user->maximum>$user->count_used){
                                ...
                                $mess = "Yêu cầu sử dụng được chấp nhận từ:\n"
                                    . "<b>Email: </b>" . "$request->email\n"
                                    . "<b>activeKey: </b>" . "$request->activeKey\n"
                                    . "<b>key: </b>" . "$request->key\n"
                                    . "<b>Đã dùng: </b>" . "$user->count_used/$user->maximum.\n";
                                $this->sendMessageToTelegram($mess);
                                $res = [
                                    'rc'=>'0',
                                    'accept'=>true,
                                ...
                                $mess = "Yêu cầu sử dụng bị từ chối do key người dùng hết lượt sử dụng từ:\n"
                                    . "<b>Email: </b>" . "$request->email\n"
                                    . "<b>activeKey: </b>" . "$request->activeKey\n"
                                    . "<b>key: </b>" . "$request->key\n"
                                    . "<b>Đã dùng: </b>" . "$user->count_used/$user->maximum.\n";
                            ...
                            $mess = "Yêu cầu sử dụng bị từ chối do key không hợp lệ từ:\n"
                                . "<b>Email: </b>" . "$request->email\n"
                                . "<b>activeKey: </b>" . "$request->activeKey\n"
                                . "<b>key: </b>" . "$request->key\n";
                            $this->sendMessageToTelegram($mess);
                        ...
                        $mess = "Yêu cầu sử dụng bị từ chối do tài khoản chưa được phê duyệt từ:\n"
                            . "<b>Email: </b>" . "$request->email\n"
                            . "<b>activeKey: </b>" . "$request->activeKey\n"
                            . "<b>key: </b>" . "$request->key\n";
                        $this->sendMessageToTelegram($mess);
                    ...
                    $mess = "Yêu cầu sử dụng bị từ chối do không tìm thấy người dùng từ:\n"
                        . "<b>Email: </b>" . "$request->email\n"
                        . "<b>activeKey: </b>" . "$request->activeKey\n"
                        . "<b>key: </b>" . "$request->key\n";
                    $this->sendMessageToTelegram($mess);
            ...
            $mess = "Có yêu cầu sử dụng code từ nguồn không xác định";
            $this->sendMessageToTelegram($mess);
            $res = [
                'rc'=>'-1',
                'accept'=>false,
